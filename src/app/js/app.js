(function ($) {
  ...
  function init_scroll() {
    if (!is_resize) {
      var window_height = $("#main-header").height() - ($("#main-menu").height() + 1);
    ...
    if ($el.length && $alert_wrap.length) {
      $el.on("submit", function () {
        ...
        $.post("src/php/sendmail.php", params, function (data) {
          ...
          if (dt.status == "error") {
            var alert = init_alert("contact-alert-err", dt.status_desc, "uk-alert-danger", "warning");
          } else {
            var alert = init_alert("contact-alert-success", dt.status_desc, "uk-alert-primary", "info");
            $el.trigger("reset");
          }
